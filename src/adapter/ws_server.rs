...
pub fn run_session(mut stream: TcpStream, base_url: &str, cfg: SessionConfig, room: SharedRoom, room_id: String, silent: bool) {
    ...
    // load models
    match crate::adapter::ollama::list_models(base_url) {
    ...
    // if username wasn't explicit, use the model name (without :version) as identity
    if !cfg.explicit_username && !cfg.tui_client {
        app.username = crate::app::model_display_name(&model_name).to_string();
    }
    ...
    // tracks the room message count we have already checked for mentions (avoid double-respond)
    let mut last_mention_checked_len: usize = 0;
    ...
    app.shared_room = Some(room.clone());
    ...
    loop {
        ...
        if current_version != known_version {
            ...
            let new_msgs: Vec<serde_json::Value> = {
                let r = room.lock().unwrap();
                r.messages[prev_len..].iter().map(|m| serde_json::json!({
                    "role":    m.role.to_api_str(),
                    "content": m.content,
                    "user":    m.tool_call.as_deref().unwrap_or(""),
                })).collect()
            ...
            // check new User messages (from others) that mention this session
            if app.stream_state == StreamState::Idle {
                let new_user_msgs: Vec<(String, String)> = app.messages[last_mention_checked_len..]
                    .iter()
                    .filter(|m| m.role == Role::User)
                    .filter(|m| m.tool_call.as_deref().map(|s| s != app.display_username()).unwrap_or(true))
                    .map(|m| (m.content.clone(), m.tool_call.clone().unwrap_or_default()))
                    .collect();

                last_mention_checked_len = app.messages.len();

                let should_respond = new_user_msgs.iter()
                    .any(|(content, _)| {
                        let mentions = crate::app::extract_mentions(content);
                        mentions.is_empty() // no mention = broadcast to all
                            || crate::app::is_mentioned(&app.display_username(), content)
                    });
            ...
            if let Some(ref msg) = chunk.message {
                ...
                if let Some(call) = maybe_call {
                    if let Some(last) = app.messages.last_mut() {
                        ...
                        // Mirror the app.rs/headless.rs truncation: drop anything
                        // the model generated after </tool>, so the next stream
                        // doesn't resume a wrong analysis written before the
                        // tool result was available.
