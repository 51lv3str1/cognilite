        ...
        2 => {
            ...
            // Token summary line
            {
                ...
                let summary = if app.neuron_sub_section == 1 {
                    // Smart: split initial (always loaded) vs on-demand (manifest only)
                    let i_tok: u64 = enabled.iter()
                        .filter(|n| !app.on_demand_neurons.contains(&n.name))
                        .map(|n| (n.system_prompt.len() / 4) as u64).sum();
                    let o_tok: u64 = enabled.iter()
                        .filter(|n| app.on_demand_neurons.contains(&n.name))
                        .map(|n| (n.system_prompt.len() / 4) as u64).sum();
                    let o_count = enabled.iter()
                        .filter(|n| app.on_demand_neurons.contains(&n.name))
                        .count();
                    if o_count > 0 {
                        format!("~{i_tok}tok active  +  ~{o_tok}tok on-demand ({o_count})")
                    } else {
                        format!("~{i_tok}tok active")
                    }
                } else {
            ...
            match app.neuron_sub_section {
                0 => {
                    ...
                    let filtered: Vec<(usize, &crate::domain::neuron::Neuron)> = app.neurons.iter().enumerate()
                ...
                1 => {
                    // Smart — unified list. Enter/Space cycles: off → initial → on-demand → off.
                    // ● = initial (in system prompt)   ◈ = on-demand (in manifest)   ○ = off
                    let filtered: Vec<(usize, &crate::domain::neuron::Neuron)> = app.neurons.iter().enumerate()
                        .filter(|(_, n)| crate::app::fuzzy_match(&app.config_search, &n.name))
                        .collect();
                    for (row, (orig_idx, neuron)) in filtered.iter().enumerate() {
                        let cursor    = *orig_idx == app.neuron_cursor;
                        let disabled  = app.disabled_neurons.contains(&neuron.name);
                        let on_demand = app.on_demand_neurons.contains(&neuron.name);
                        let (marker, mark_fg) = if disabled {
                            ("○", DIM)
                        } else if on_demand {
                            ("◈", THINKING_COLOR)
                        } else {
                        ...
                        let tok = (neuron.system_prompt.len() / 4) as u64;
                        frame.render_widget(Paragraph::new(Line::from(vec![
                            Span::styled(format!("  {marker} "), bg.patch(Style::default().fg(mark_fg))),
                            Span::styled(&neuron.name, name_style),
                            Span::styled(desc, desc_style),
                            Span::styled(format!("  ~{tok}tok"), bg.patch(Style::default().fg(THINKING_COLOR))),
                        ])), Rect { x: inner.x, y: content_y + row as u16, width: inner.width, height: 1 });
...
fn draw_model_select(frame: &mut Frame, app: &App) {
    ...
    if app.loading_models {
        ...
        let filtered_models: Vec<(usize, &crate::adapter::ollama::ModelEntry)> = app.models.iter().enumerate()
