...
pub fn run(base_url: &str, args: HeadlessArgs) -> i32 {
    ...
    if args.yes { app.auto_accept = true; } // --yes also bypasses the destructive-shell gate
    ...
    app.model_template = crate::adapter::ollama::fetch_template(base_url, &model_name);
        ...
        loop {
            ...
            if let Some(ref msg) = chunk.message {
                ...
                if let Some(call) = maybe_call {
                    ...
                    if let Some(ask) = app.ask.take() {
                        let response = ask_interactive(&ask.kind, &ask.question, auto_yes);
                        app.ask = Some(ask);
